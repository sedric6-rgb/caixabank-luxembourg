...
export default function AccountView({ client, initialAccount }: { client: AdminClient; initialAccount: DemoAccount }) {
  ...
  const TYPE_LABELS: Record<string, string> = { courant: "Compte Courant", epargne: "Livret Epargne", professionnel: "Compte Pro" };
  ...
  const addTransaction = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await adminAddTransactionAction(client.id, account.number, newTx);
    if (!res.success) { notify("Erreur lors de l'enregistrement"); return; }
  ...
  const updateTransaction = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const signedAmount = newAmount >= 0 ? Math.abs(newAmount) : -Math.abs(newAmount);
    const diff = signedAmount - oldTx.amount;
    const idx = editIdx;
    const updated = { date: newDate, desc: newDesc, amount: signedAmount };
    startTransition(async () => {
      const res = await adminUpdateTransactionAction(client.id, account.number, idx, oldTx, updated);
      if (!res.success) { notify(res.error); return; }
      setTransactions((prev) => prev.map((tx, i) => i === idx ? updated : tx));
  ...
  const deleteTransaction = (idx: number) => {
    ...
    startTransition(async () => {
      const res = await adminDeleteTransactionAction(client.id, account.number, idx, tx);
      if (!res.success) { notify(res.error); return; }
  ...
  return (
    <div>
      ...
      {/* Header */}
      <div className="mb-4">
        <Link href="/admin/comptes" className="text-sm text-blue-600 hover:underline mb-2 inline-block">&larr; Retour</Link>
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{TYPE_LABELS[account.type] || account.type}</h1>
            <p className="text-xs font-mono text-gray-500 mt-1 break-all">{account.number}</p>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-4 overflow-x-auto">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`px-3 sm:px-4 py-2 rounded-md text-sm font-medium whitespace-nowrap transition-colors ${tab === t.key ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
      ...
      {tab === "mouvements" && (
        <div className="bg-white rounded-xl border border-gray-200">
          <div className="flex items-center justify-between p-4 border-b border-gray-200">
            <h2 className="font-semibold text-gray-900 text-sm sm:text-base">Mouvements</h2>
            <button onClick={() => setShowAddTx(true)} className="bg-[#003d82] text-white px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium hover:bg-[#002a5c]">
              + Ajouter
            </button>
          </div>
          ...
          {showAddTx && (
            <form onSubmit={addTransaction} className="p-4 border-b border-gray-200 bg-blue-50/50">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowAddTx(false)} className="text-sm text-gray-500 px-3">Annuler</button>
              </div>
            </form>
          )}

          {transactions.length === 0 ? (
            <p className="p-6 text-center text-gray-400 text-sm">Aucun mouvement</p>
          ) : (
                  ...
                  {editIdx === i ? (
                    <form onSubmit={updateTransaction} className="space-y-3">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input name="date" defaultValue={tx.date} required className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Montant</label>
                          <input name="amount" type="number" step="0.01" defaultValue={tx.amount} required className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm shrink-0 ${tx.amount >= 0 ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
                          {tx.amount >= 0 ? "+" : "-"}
                        </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button onClick={() => deleteTransaction(i)} className="p-1.5 rounded-md hover:bg-red-100 text-red-400" title="Supprimer">
      ...
      {tab === "cartes" && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
          ...
          {cards.length === 0 ? (
            <p className="text-sm text-gray-400">Aucune carte</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {cards.map((c, i) => (
                <div key={i} className={`rounded-xl p-4 sm:p-5 text-white ${c.status === "Active" ? "bg-gradient-to-br from-gray-800 to-gray-900" : "bg-gradient-to-br from-red-800 to-red-900"}`}>
                  ...
                  <p className="text-base sm:text-lg font-mono tracking-widest mb-4">**** **** **** {c.last4}</p>
...
function Info({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-gray-500">{label}</dt><dd className="font-medium text-gray-900 mt-0.5 break-all">{value}</dd></div>;
}
