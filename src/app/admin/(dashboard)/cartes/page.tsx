  ...
  for (const c of DEMO_CLIENTS) {
    ...
    for (const card of c.cards) {
      cards.push({
        ...
        type: card.type,
        account: acctShort,
        status: card.status === "active" ? "active" : card.status === "bloquee" || card.status === "bloquée" ? "bloquee" : card.status,
        expiry: card.expiry,
      });
    }
  }
...
const ALL_CLIENTS = DEMO_CLIENTS.filter((c) => c.accounts.length > 0).map((c) => ({
...
export default function AdminCartesPage() {
  ...
  const [selectedClient, setSelectedClient] = useState(ALL_CLIENTS[0]?.name || "");
  ...
  const clientAccounts = ALL_CLIENTS.find((c) => c.name === selectedClient)?.accounts || [];
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <select name="client" value={selectedClient} onChange={(e) => setSelectedClient(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                {ALL_CLIENTS.map((c) => <option key={c.name} value={c.name}>{c.name}</option>)}
              </select>
            </div>
            <div>
