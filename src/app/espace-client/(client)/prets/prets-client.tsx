...
type Loan = {
  id: number; type: string; amount: number; rate: number; duration: number;
  monthly: number; remaining: number; status: string; start: string; end: string;
};

export default function PretsClient({ initialLoans }: { initialLoans: Loan[] }) {
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">Credits & Prets</h1>
      <p className="text-sm text-gray-500 mb-6">Gerez vos credits et simulez de nouveaux prets</p>

      {initialLoans.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Mes credits en cours</h2>
          <div className="space-y-4">
            {initialLoans.map((loan) => {
              const progress = ((loan.amount - loan.remaining) / loan.amount) * 100;
              return (
                <div key={loan.id} className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Simuler un credit</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Duree : <span className="font-bold">{Math.floor(simDuration / 12)} ans ({simDuration} mois)</span></label>
