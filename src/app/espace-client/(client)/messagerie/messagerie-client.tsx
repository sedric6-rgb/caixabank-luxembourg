...
type Message = { id: number; subject: string; body: string; sender: string; read: boolean; date: string };

export default function MessagerieClient({ initialMessages }: { initialMessages: Message[] }) {
    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">{replyTo ? `Re: ${replyTo.subject}` : "Nouveau message"}</h1>
        </div>
        <form onSubmit={sendMessage} className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 max-w-2xl space-y-4">
          <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden divide-y divide-gray-100">
        {messages.map((msg) => (
          <button key={msg.id} onClick={() => openMessage(msg.id)}
            className={`w-full text-left flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-4 hover:bg-gray-50 transition-colors ${!msg.read ? "bg-blue-50/50" : ""}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {msg.sender === "client" && <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded shrink-0">Envoye</span>}
              </div>
              <p className="text-xs text-gray-400 truncate">{msg.body.slice(0, 80)}...</p>
            </div>
            <span className="text-xs text-gray-400 whitespace-nowrap shrink-0">{msg.date}</span>
          </button>
        ))}
        {messages.length === 0 && (
          <div className="px-6 py-8 text-center text-gray-400 text-sm">Aucun message</div>
        )}
      </div>
    </div>
  );
