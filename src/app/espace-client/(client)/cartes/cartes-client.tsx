...
export default function CartesClient({ initialCards }: { initialCards: Card[] }) {
  ...
  const opposeCard = (card: Card) => {
    ...
    setConfirm(null);
    notify(`Opposition enregistree pour la carte **** ${card.last4}`);
  };
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-3 border-t border-gray-200 flex gap-2">
                    {card.status === "active" ? (
                      <>
                        <button onClick={() => setConfirm({ card, action: "block" })} className="text-xs px-4 py-2 rounded-lg bg-red-50 text-red-700 font-medium hover:bg-red-100">Bloquer</button>
                        <button onClick={() => setConfirm({ card, action: "oppose" })} className="text-xs px-4 py-2 rounded-lg bg-gray-100 text-gray-700 font-medium hover:bg-gray-200">Opposer</button>
                      </>
                    ) : card.status === "blocked" ? (
                      <button onClick={() => blockCard(card)} className="text-xs px-4 py-2 rounded-lg bg-green-100 text-green-700 font-medium hover:bg-green-200">Debloquer</button>
                    ) : (
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-600 mb-6">
              {confirm.action === "block"
                ? `Bloquer temporairement la carte **** ${confirm.card.last4} ? Vous pourrez la debloquer.`
                : `Mettre en opposition la carte **** ${confirm.card.last4} ? Cette action est irreversible.`
              }
            </p>
            <div className="flex gap-3">
              <button onClick={() => confirm.action === "block" ? blockCard(confirm.card) : opposeCard(confirm.card)}
                ...
                {confirm.action === "block" ? "Bloquer" : "Confirmer"}
              </button>
